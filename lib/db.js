...
export const getReelsInfo = (Reel, setInfo) =>
  onSnapshot(doc(db, "reels", Reel.id), (doc)=>{
    setInfo(doc.data())
  });
...
export const getReels = async () => {
  ...
  docSnap.forEach((doc) => reelArr.push(doc.data()))
  return reelArr
}
...
export const handleLikeReels = async (post) => {
  const userId = auth.currentUser.uid;
  const currentLikeStatus = !post.likes_by_users.includes(userId);
  await updateDoc(doc(db, "reels", post.id), {
    likes_by_users: currentLikeStatus
      ? arrayUnion(userId)
      : arrayRemove(userId),
  });
